...
def dYdt_2D_moving_moon(t, Y):
    ...
    a1 = -G * m1 * r3_vec / (r3_mag**3) if r3_mag > 1e-6 else np.zeros(2)
...
# dYdt_2D = dYdt_2D_fixed_moon 
dYdt_2D = dYdt_2D_moving_moon  


USE_MOVING_MOON = (dYdt_2D is dYdt_2D_moving_moon)
...
def adams_bashforth_4th_order_vec(f, t_init, Y_init, h, total_steps):
    ...
    for n in range(3, total_steps):
        ...
        Y_n = Y_values[n]
        Y_next = Y_n + (h / 24) * (
            55 * F_n - 59 * F_n_minus_1 + 37 * F_n_minus_2 - 9 * F_n_minus_3
        )
        t_next = t_values[n] + h
        t_values.append(t_next)
        Y_values.append(Y_next)
